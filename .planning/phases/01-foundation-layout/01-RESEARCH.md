...
1. **create-next-app `--yes` src directory default**
   - RESOLVED: Use explicit flags (`--src-dir`) to guarantee the src directory structure matching DESIGN.md. Do not rely on `--yes` defaults.

2. **Zustand persist + storage adapter interaction**
   - RESOLVED: Use Zustand persist for in-memory state persistence (auto-save on every change). Use the StorageAdapter for explicit save/load operations (export/import, future database). They serve different purposes and coexist.

3. **AGENTS.md generated by create-next-app**
   - RESOLVED: Keep it if useful. CLAUDE.md takes precedence per project instructions.
